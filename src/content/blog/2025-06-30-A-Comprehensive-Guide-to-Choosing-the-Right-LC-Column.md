...
- **Type A Silica:** The original generation of silica particles, produced by gelling silicic acid. Their residual metal content and acidic silanols can cause peak tailing with basic analytes, which is why modern bonded phases are built on the higher-purity material below.
- **Type B Silica:** Today's standard, manufactured from organosilanes with significantly lower metal content and fewer acidic silanols. Type B silica delivers better peak symmetry for basic compounds and improved batch-to-batch reproducibility.
...
4. **Agilent HPLC & LC Column Selector Tool** (Agilent): This tool from Agilent provides solutions for choosing a column by USP designation or agency methods from the EPA and ASTM. [Source](https://www.agilent.com/search/gn/lc-column-selector)
